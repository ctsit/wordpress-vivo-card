...
WHERE {
        ...
        OPTIONAL {  <$personURI> obo:ARG_2000028 ?contact_info . ?contact_info vcard:hasEmail ?email . ?email vcard:email ?emailaddr .  }
        OPTIONAL {?contact_info vcard:hasTelephone ?phone . ?phone vcard:telephone ?telephone . }
        OPTIONAL {?contact_info vcard:hasTelephone ?fax_num . ?fax_num a vcard:Fax . ?fax_num vcard:telephone ?actual_fax_num}
        OPTIONAL {?contact_info vcard:hasTitle ?title . }
        OPTIONAL {?title vcard:title ?title_label . }
        OPTIONAL {<$personURI> ufVivo:homeDept ?dept . ?dept rdfs:label ?deptLabel . }
        OPTIONAL {<$personURI> vitro:mainImage ?image. ?image vitro:thumbnailImage ?thumb . ?thumb vitro:downloadLocation ?photo}
} LIMIT 1 ";
        ...
        $vivoTitle = $sparqlArray["results"]["bindings"][0]["title_label"]["value"];
        ...
        $image_url = $sparqlArray["results"]["bindings"][0]["photo"]["value"];
        $vivoImage = self::get_image_path_from_image_url($image_url);
        // echo "$personURI, $vivoImage";
